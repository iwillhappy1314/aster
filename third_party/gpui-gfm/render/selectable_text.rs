...
impl SelectableText {
    ...
    fn ensure_runs_up_to_date(&mut self) {
        let selection =
            cross_block_selection_range(&self.selection_state, self.text_id, self.text.as_ref());
    ...
    fn paint(
        ...
        window.on_mouse_event({
            ...
            move |event: &MouseMoveEvent, phase, window, _cx| {
                ...
                let updated =
                    selection_with_head(active, text_id, index, text_for_move.as_ref(), true);
        ...
        window.on_mouse_event({
            ...
            move |event: &MouseUpEvent, phase, window, cx| {
                ...
                let updated =
                    selection_with_head(active, text_id, index, text_for_up.as_ref(), false);
                set_cross_block_selection(&selection_state, updated);
...
fn selection_with_head(
    ...
    match active.mode {
        SelectionMode::Word => {
            ...
            active.anchor = if before_anchor {
                initial.end
            } else {
                initial.start
            };
            active.head = if before_anchor {
                current.start
            } else {
                current.end
            };
