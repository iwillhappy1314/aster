        ...
        match segment {
            MarkdownPreviewSegment::Markdown(markdown) => {
                if !markdown.is_empty() {
                    ...
                    container =
                        container.child(blocks::render_blocks(parsed.blocks(), options, 0, cx));
                }
            }
...
mod tests {
    ...
    fn selection_state_with_links_renders(cx: &mut gpui::TestAppContext) {
        let options =
            MarkdownRenderOptions::default().with_on_link(Arc::new(|_url, _window, _cx| {}));
        cx.update(|cx| {
    ...
    fn selection_ids_unique_across_preview_segments(cx: &mut gpui::TestAppContext) {
        ...
        let preview_url: Arc<str> =
            "https://github.com/owner/repo/blob/abc123/file.rs#L1-L3".into();
    ...
    fn override_table_is_called(cx: &mut gpui::TestAppContext) {
        ...
        cx.update(|cx| {
            let _ =
                crate::render::render_markdown("| A | B |\n|---|---|\n| 1 | 2 |\n", &options, cx);
        });
        assert_eq!(counter.load(Ordering::Relaxed), 1);
    }
